...
class ScopeCapture(QWidget):
    def __init__(self, parent=None):
        ...
        self.pbChThree.clicked.connect(
            lambda: self.on_btn_clicked(self.pbChThree))
        self.pbChFour = self.setup_button("4")
        self.pbChFour.clicked.connect(
            lambda: self.on_btn_clicked(self.pbChFour))
    ...
    def on_pb_clicked(self):
        ...
        for item in self.listOfButtons:
            ...
            data = raw_data_to_string(self.scope.get_data(int(item.text())))
            # print(data)
            self.append_csv(myfile, tmpfile, item, data)
            copyfile(tmpfile, myfile)
        #waveform = raw_data_to_string(self.scope.get_data())
        # print(waveform)
        filename = "_"+str(self.cb.currentText())+"_" + \
            datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        filename = self.createTempFile(filename)
        copyfile(myfile, filename+".csv")
        # with open(filename+".csv", 'w') as f:
        # for point in waveform:
        # f.write(point)
        try:
            self.myBucket.put_object(
                Key=filename+'.csv', Body=open(filename+'.csv', 'rb'))
        ...
        try:
            self.myBucket.put_object(
                Key=filename+'.png', Body=open(filename+'.png', 'rb'))
